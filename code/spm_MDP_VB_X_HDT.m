%% Version of the standard spm_MDP_VB_X.m SPM routine used for the heartbeat discrimination task (HDT)

function [MDP] = spm_MDP_VB_X_HDT(MDP,OPTIONS)

% active inference and learning using variational message passing
% FORMAT [MDP] = spm_MDP_VB_X_HDT(MDP,OPTIONS)
%
    ...
    for i = 1:size(MDP,2)                 
        for m = 1:size(MDP,1)          
            if i > 1                       % if previous inversions
                ...
                % CS: Save MDP.MDP.a{1,1} (used for IP1Diff)
                if isfield(MDP,'MDP')
                    lower_a = MDP(m,i).MDP.a{1,1};
                end
                ...
                % CS: If using IP1Diff, undo the updating of
                % MDP.MDP.a{1,1} and use the prescribed value instead
                %----------------------------------------------------------
                fit_options = MDP(1).fit_options;

                if isfield(fit_options,'IP1Diff')
                    if fit_options.IP1Diff ==1
                        MDP(m,i).MDP.a{1,1} = lower_a;
                    end
                end

                % CS: if using omegaBlock (forgetting between sessions)
                %----------------------------------------------------------
                if isfield(fit_options,'forgettingBlock')
                    if fit_options.forgettingBlock == 1
                        try omegaBlock = MDP(1).omegaBlock; catch, omegaBlock = 1;    end % forgetting rate
                        if MDP(m,i).block > MDP(m,i - 1).block % i.e., if block value goes up from last trial
                           MDP(m,i).a{1, 1} = MDP(m,i).a{1, 1}*omegaBlock; % scale down a matrix for this trial only
                        elseif MDP(m,i).block == MDP(m,i - 1).block % if block value remains the same
                        else
                            disp('block number problem') % if anything else
                        end
                        clear omegaBlock
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_X_HDT(MDP(:,i),OPTIONS);
...
% CS: initialise learning rates for initial state bias and likelihoods
try etaA  = MDP(1).etaA;  catch, etaA  = 1;    end 
try etaD  = MDP(1).etaD;  catch, etaD  = 1;    end 
try etaLower  = MDP(1).etaLower;  catch, etaLower  = 1; end 
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'link')
            ...
            %==============================================================
            MDP(m).mdp(t) = spm_MDP_VB_X_HDT(mdp);
...
for m = 1:size(MDP,1)
    ...
    for t = 1:T     % T = 1 for lower-level a. T = 3 for higher-level a.
                    %MDP.action is coded 1 vs. 2 (Async vs. Sync) because
                    %the corresponding state factor has two possible states


        % mapping from hidden states to outcomes: a
        %------------------------------------------------------------------

        % CS: Updating likelihood matrix depending on hierarchical set-up
        if isfield(MDP,'a')
            for g = 1:Ng(m)
                da     = O{m}(g,t);
                for  f = 1:Nf(m)
                ...
                if isfield(MDP, 'link') % if updating higher-level MDP, which has the field 'link'
                    MDP(m).a{g} = MDP(m).a{g}*omega + da*etaA; % Single learning rate etaA

                    % Prevent forgetting from reducing dirichlet counts 
                    % below their starting values (lower bound for forgetting).
                    MDP(m).a{g}(2,1,2) = max(MDP(m).a{g}(2,1,2), MDP.IP2);
                    MDP(m).a{g}(3,2,2) = max(MDP(m).a{g}(3,2,2), MDP.IP2);
                    MDP(m).a{g}(3,1,2) = max(MDP(m).a{g}(3,1,2), 1 - MDP.IP2);
                    MDP(m).a{g}(2,2,2) = max(MDP(m).a{g}(2,2,2), 1 - MDP.IP2);
                    
                elseif ~isfield(MDP,'fit_options') % if updating lower-level MDP, in a hierarchical model
                    MDP(m).a{g} = MDP(m).a{g}*omega + da*etaLower;

                elseif isfield(MDP,'fit_options') % if updating in a non-hierarchical model (only one MDP)
                    MDP(m).a{g} = MDP(m).a{g}*omega + da*etaA;
    ...
    if isfield(MDP,'d')
        for f = 1:Nf(m)
            i = MDP(m).d{f} > 0;
            MDP(m).d{f}(i) = MDP(m).d{f}(i)*omega + X{m,f}(i,1)*etaD;
        end
    end
