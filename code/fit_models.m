...
for model = 1:length(field)  
    parfor i = 1:length(subjects) % To use parallel processing, change to parfor
        tic
        rng("default")
        HDT_batch_fit(subjects{i}, field{model}, fit_options(model), data_dir, results_dir{model});
        toc
    end    
...
function output = HDT_batch_fit(subject, field, fit_options, data_dir, results_dir)
